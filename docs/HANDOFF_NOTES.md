...
**Phase 1: Backend (6/6 tasks) ** 4. **Task 1.1**: YOLO-only snapshot detection endpoint (`/detect-snapshot`) 5. **Task 1.2**: Auto-scan service (`autoScanService.ts`) 6. **Task 1.3**: Auto-scan API routes (start/stop/status) 7. **Task 1.4**: Snapshot detection integrated with auto-scan 8. **Task 1.5**: Recording integrated with auto-scan 9. **Task 1.6**: WebSocket events for progress

**Phase 2: Frontend (4/5 tasks) ** 10. **Task 2.1**: AutoScanDialog progress modal component 11. **Task 2.3**: Auto-scan button in PrimaryVideoPlayer 12. **Task 2.4**: WebSocket listeners in AutoScanDialog
