...
### 1.1 ChunkProcessor ✅ COMPLETE (with known issue)
...
- [~] Test standalone processing (blocked by Docker codec issue)

**Known Issue**: OpenCV VideoWriter fails with all codecs (mp4v, XVID, X264) in Docker container
**Next Step**: Implement FFmpeg-based video writing as workaround (Phase 1.1.1)
...
**Current Status**:
Phase 1.1 and 1.2 are **95% complete** - Implementation done, but blocked by Docker codec issue.
...
**Next Priority**:
**Phase 1.1.1 - Fix OpenCV VideoWriter codec issue**

**CRITICAL BLOCKER**: OpenCV VideoWriter fails with all codecs in Docker:
- Tried: mp4v → XVID → X264
- Error: `Could not open codec h264_v4l2m2m, error: Unspecified error (-22)`
- Error: `Failed to create video writer - all codecs failed`

**Solution Options**:
1. **Option A (Recommended)**: Use FFmpeg via subprocess to write video
   - Save frames as temporary PNGs
   - Use FFmpeg to create MP4 from frame sequence
   - More reliable in Docker containers

2. **Option B**: Install additional codec libraries in Docker
   - Add `libx264-dev`, `libxvidcore-dev` to Dockerfile
   - May still have compatibility issues

3. **Option C**: Output frames as image sequence + metadata
   - Frontend reconstructs video from frames
   - Not ideal for user experience

**Recommended Next Steps**:
1. Implement FFmpeg-based video writing in ChunkProcessor
2. Create temp directory for frames: `/tmp/chunk_processing/{chunk_id}/`
3. Save each processed frame as PNG
4. Use subprocess to call FFmpeg: `ffmpeg -framerate {fps} -i frame_%04d.png -c:v libx264 output.mp4`
5. Clean up temp frames after video creation
6. Test with Docker ml-service

**Blockers/Issues**:
- OpenCV VideoWriter codec compatibility in Docker (blocking video output testing)
- Once fixed, need to test: video output, JSON output, endpoint response
...
- api-gateway: unhealthy ⚠️ (not blocking ML work)
- video-streamer: healthy ✅
- stream-service: healthy ✅
- frontend: running ✅

ML Service logs show:
- YOLO models loaded successfully (yolov5m.pt)
- RTMPose model loaded successfully
- Endpoint `/api/process-chunk` responding (500 due to codec issue)
...
1. Implement FFmpeg-based video writing to fix codec issue
2. Test the complete flow: process chunk → output video + JSON
3. Verify JSON structure matches expected schema
4. Move to Phase 1.3: Verify models in Docker
5. Then continue to Phase 2: API Gateway Integration
